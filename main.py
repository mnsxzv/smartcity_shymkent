...
async def analyze_text_with_ai(user_text):
    ...
    # Жесткий фильтр для известных объектов города (с обновленными категориями)
    if "акимат" in text_lower:
        ...
        return "Дороги", "проспект Аль-Фараби", 42.3198, 69.5750, 500
    ...
    microrayons = { 
        ...
        "бозарык": ("мкр. Boзарык", 42.427946, 69.624233, 1100),
        ...
        "спорт": ("Спортивный мкр.", 42.335862, 69.599538, 400),
    ...
    # Обновленный маппинг ключевых слов под новые расширенные категории
    if found_key:
        name, lat, lng, radius = microrayons[found_key]
        category = "Безопасность" # Дефолтная категория для неопределенных
        
        if "свет" in text_lower or "фонар" in text_lower or "освещен" in text_lower:
            category = "Освещение"
        elif "ям" in text_lower or "асфальт" in text_lower or "открытый люк" in text_lower:
            category = "Дороги"
        elif "автобус" in text_lower or "маршрут" in text_lower or "светофор" in text_lower or "знак" in text_lower:
            category = "Транспорт"
        elif "мусор" in text_lower or "бак" in text_lower or "свалка" in text_lower:
            category = "Мусор"
        elif "дерев" in text_lower or "спил" in text_lower or "воздух" in text_lower:
            category = "Экология"
        elif "вода" in text_lower or "отоплен" in text_lower or "труб" in text_lower or "газ" in text_lower or "электрич" in text_lower:
            category = "ЖКХ"
        elif "парк" in text_lower or "детск" in text_lower or "площадк" in text_lower or "скамейк" in text_lower or "тротуар" in text_lower:
            category = "Благоустройство"
        elif "собак" in text_lower or "чердак" in text_lower or "подвал" in text_lower:
            category = "Безопасность"
            
        return category, name, lat, lng, radius

    # Основной промпт для нейросети с жестко заданной расширенной палитрой категорий
    prompt = f"""
    Ты — ИИ-модуль Ситуационного центра Шымкента. Твоя задача — извлечь из текста расширенную категорию, точный адрес, координаты центра (lat, lng), а ТАКЖЕ определить приблизительный РАДИУС зоны происшествия в метрах.
    
    Выбери ОДНУ максимально подходящую категорию СТРОГО из следующего списка:
    1. "Освещение" — нерабочие фонари, темнота во дворах, сломанные столбы.
    2. "Дороги" — ямы, провалы в асфальте, открытые люки, отсутствие разметки.
    3. "Транспорт" — жалобы на автобусы, интервалы, неработающие светофоры, дорожные знаки.
    4. "Мусор" — стихийные свалки, переполненные контейнеры, неубранная территория.
    5. "Экология" — спил деревьев, вырубка, загрязнение воздуха или водоемов.
    6. "ЖКХ" — прорыв труб, отсутствие воды/отопления/газа, проблемы с электросетями.
    7. "Благоустройство" — сломанные детские площадки, разбитые скамейки, ремонт тротуаров, брусчатки.
    8. "Безопасность" — бродячие собаки, открытые подвалы/чердаки, опасные разрушения конструкций.

    Правила для поля "radius" (в метрах):
    - Если указан точный объект, ТЦ, или конкретный дом (например: ул. Рыскулова 45, ТРЦ Плаза) -> ставь radius от 50 до 150.
    - Если указана просто длинная улица без номера дома -> ставь radius от 400 до 600.

    Верни СТРОГО чистый JSON без markdown разметки (без ```json ... ```):
    {{"category": "Одна категория из 8 разрешенных выше", "address": "адрес в Шымкенте", "lat": 42.3417, "lng": 69.5901, "radius": 200}}
    ...
    try:
        ...
        # Валидация категории: страховка от выдумок ИИ
        allowed_cats = ["Освещение", "Дороги", "Транспорт", "Мусор", "Экология", "ЖКХ", "Благоустройство", "Безопасность"]
        ai_cat = data.get("category", "Безопасность")
        if ai_cat not in allowed_cats:
            ai_cat = "Безопасность"
        ...
        return "Безопасность", "Шымкент", 42.3417, 69.5901, 300
